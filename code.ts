...
const getArtwork = async () => { 
  ...
  if (!selected) return; try { return selected.exportAsync({format: 'PNG'})
...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'get-image') {
    getArtwork().then(r => { 
      if (!r) figma.ui.postMessage(null); // empty message ; 
      else figma.ui.postMessage({ data: r.data, UUID: "Xxxxx", access: "access", type: 'PICTURE' }); // message 
      })
 }
